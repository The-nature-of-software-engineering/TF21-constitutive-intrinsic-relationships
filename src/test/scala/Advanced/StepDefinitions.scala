...
class StepDefinitions extends ScalaDsl with EN {
  ...
  When("""{word} pays {word} {double} dollars""") { (debtor: String, creditor: String, amount: Double) =>
    ...
    val result: Either[String, PaymentResult] =
      debtorAccount.actingAsADebitingAccount() pays (creditorAccount.actingAsACreditingAccount()) theAmountOf amount

    accounts(debtor) = result.map(_.debitingAccountRole.account).getOrElse(Account("unknown", 0.0))
    accounts(creditor) = result.map(_.creditingAccountRole.account).getOrElse(Account("unknown", 0.0))
  }
